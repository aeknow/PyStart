...
from pystart.common import Record, UserError, normpath_with_actual_case
from pystart.config import try_load_configuration
from pystart.config_ui import ConfigurationDialog
from pystart.custom_notebook import CustomNotebook, CustomNotebookPage
from pystart.editors import Editor, EditorNotebook
...
from pystart.program_analysis import ProgramAnalyzer
from pystart.running import BackendProxy, Runner
from pystart.shell import ShellView
...
OBSOLETE_PLUGINS = [
    "pystartcontrib.pi",
    "pystartcontrib.micropython",
    "pystartcontrib.circuitpython",
    "pystartcontrib.microbit",
    "pystartcontrib.esp",
    "pystartcontrib.rpi_pico",
]


class Workbench(tk.Tk):
    """
    PyStart's main window and communication hub.
    ...
    def __init__(self, parsed_args: Dict[str, Any]) -> None:
        ...
        tk.Tk.__init__(self, className="PyStart")
        ...
        for name in sorted(sys.modules):
            if (
                ...
                and not name.startswith("pystart")
    ...
    def _make_sanity_checks(self):
        ...
        if bad_home_msg:
            messagebox.showwarning(
                ...
                bad_home_msg + "\nThis may cause problems for PyStart.",
                master=self,
            )
    ...
    def _init_configuration(self) -> None:
        self._configuration_manager = try_load_configuration(pystart.get_configuration_file())
        self._configuration_pages = []  # type: List[Tuple[str, str, Type[tk.Widget], int]]

        self.set_default("general.single_instance", pystart.SINGLE_INSTANCE_DEFAULT)
        ...
        self.set_default(
            "general.data_url_prefix", "https://raw.githubusercontent.com/pystart/thonny/master/data"
        )
    ...
    def _get_secrets_path(self) -> str:
        return os.path.join(get_pystart_user_dir(), "secrets.json")
    ...
    def update_debug_mode(self):
        os.environ["PYSTART_DEBUG"] = str(self.get_option("general.debug_mode", False))
        pystart.set_logging_level()
        ...
        for class_ in self._language_server_proxy_classes:
            ...
            ls_proxy = class_(
                InitializeParams(
                    ...
                    processId=os.getpid(),
                    clientInfo=ClientInfo(name="PyStart", version=pystart.get_version()),
    ...
    def _init_window(self) -> None:
        ...
        # even if they were True when PyStart was last run
    ...
    def _load_plugins(self) -> None:
        # built-in plugins
        import pystart.plugins  # pylint: disable=redefined-outer-name

        self._load_plugins_from_path(pystart.plugins.__path__, "pystart.plugins.")  # type: ignore

        # 3rd party plugins from namespace package
        # Now it's time to add plugins dir to sys path
        sys.path.append(pystart.get_sys_path_directory_containg_plugins())
        try:
            import pystartcontrib  # @UnresolvedImport
        except ImportError:
            # No 3rd party plugins installed
            pass
        else:
            self._load_plugins_from_path(pystartcontrib.__path__, "pystartcontrib.")
    ...
    def _create_server_socket(self):
        if running_on_windows():
            ...
            with open(pystart.get_ipc_file_path(), "w") as fp:
                fp.write(str(port) + "\n")
                fp.write(secret + "\n")

        else:
            server_socket = socket.socket(socket.AF_UNIX)  # @UndefinedVariable
            server_socket.bind(pystart.get_ipc_file_path())
            secret = ""

        os.chmod(pystart.get_ipc_file_path(), 0o600)
    ...
    def _post_backend_menu(self):
        from pystart.plugins.micropython.uf2dialog import (
            show_uf2_installer,
        ...
        if self.winfo_screenwidth() / self.winfo_screenheight() > 2:
            ...
            # Tk will adjust x properly with single monitor, but when PyStart is maximized
    ...
    def _init_theming(self) -> None:
        if self.get_option("view.ui_theme") == "Kind of Aqua":
            # replace pre-PyStart 5 theme with new theme
    ...
    def _publish_command(
        ...
        if sequence:
            if not skip_sequence_binding:
                ...
                # https://github.com/pystart/thonny/issues/1347
        ...
        def on_click(event):
            ...
            tk.messagebox.showinfo(
                ...
                tr(
                    ...
                    + "Restart PyStart to start working in regular mode.\n\n"
    ...
    def get_package_dir(self):
        ...
        return os.path.dirname(sys.modules["pystart"].__file__)
    ...
    def in_debug_mode(self) -> bool:
        return os.environ.get("PYSTART_DEBUG", False) in [
            "1",
            1,
            "True",
            True,
        ...
        if self._toolbar.winfo_ismapped():
            for group_frame in self._toolbar.grid_slaves(0):
                for button in group_frame.pack_slaves():
                    if pystart._runner is None or button.tester and not button.tester():
    ...
    def _guard_font_size(self, size: int) -> int:
        # https://bitbucket.org/plas/pystart/issues/164/negative-font-size-crashes-thonny
        MIN_SIZE = 4
        MAX_SIZE = 200
        if size < MIN_SIZE:
            return MIN_SIZE
    ...
    def destroy(self) -> None:
        try:
            ...
            if self._is_server() and os.path.exists(pystart.get_ipc_file_path()):
                os.remove(pystart.get_ipc_file_path())
    ...
    def get_temp_dir(self, create_if_doesnt_exist=True):
        path = os.path.join(get_pystart_user_dir(), "temp")
    ...
    def show_notebook_drop_targets(self):
        from pystart.custom_notebook import NotebookTabDropTarget
    ...
    def _create_new_virtual_environment(self):
        from pystart.venv_dialog import create_new_virtual_environment
    ...
    def _check_version_alignment(self):
        ...
        try:
            ...
            installation_version = version("pystart")
            embedded_version = pystart.get_version()
            if installation_version != embedded_version:
                messagebox.showwarning(
                    "Warning",
                    f"PyStart's installation version is reported as {installation_version!r},\n"
                    + f"but embedded version is {embedded_version!r}.",
                )
        except Exception:
            # Ignore metadata errors when running from source code
            # This is normal when running PyStart from source rather than installed package
            pass
