...
class ToolTip:
    """改进的工具提示类，支持延迟显示和更好的定位"""

    def __init__(self, widget, options, delay=50):
    ...
    def schedule_showtip(self, text):
        """延迟显示工具提示"""
        self.text = text
        # 取消之前的计划
        ...
        # 安排新的显示
        self._schedule_id = self.widget.after(self.delay, self._do_showtip)
        # 调试信息
        # print(f"Scheduled tooltip for {self.widget} with delay {self.delay}")

    def _do_showtip(self):
        """实际显示工具提示"""
        self._schedule_id = None
        ...
        try:
            ...
            # 获取工具提示的预估宽度和高度
            temp_label = tk.Label(None, text=self.text, **self.options)
    ...
    def hidetip(self, event=None):
        ...
        if tw:
            try:
                if self.focus_out_bind_ref:
                    self.widget.winfo_toplevel().unbind("<FocusOut>", self.focus_out_bind_ref)
                tw.destroy()
            except tk.TclError:
                pass


def create_tooltip(widget, text, delay=50, **kw):
    """创建工具提示
    
    Args:
        widget: 要添加工具提示的控件
        text: 工具提示文本
        delay: 显示延迟（毫秒），默认50ms
    """
    ...
    options.setdefault("padx", 5)
    options.setdefault("pady", 3)
    options.update(kw)

    toolTip = ToolTip(widget, options, delay=delay)

    def enter(event):
        toolTip.schedule_showtip(text)
    ...
    if hasattr(widget, 'winfo_children'):
        for child in widget.winfo_children():
            child.bind("<Enter>", enter, True)
            child.bind("<Leave>", leave, True)
            # 递归处理孙子控件
            if hasattr(child, 'winfo_children'):
                for grandchild in child.winfo_children():
                    grandchild.bind("<Enter>", enter, True)
                    grandchild.bind("<Leave>", leave, True)
...
def _check_dialog_parent(options):
    ...
    if running_on_mac_os():
        ...
        # but this is deprecated in Catalina (https://github.com/pystart/thonny/issues/840)
...
class _CustomDialogProvider:
    ...
    def _call(cls, kind: Literal["save", "open", "dir"], **options):
        from pystart.base_file_browser import (
...
def handle_mistreated_latin_shortcuts(registry, event):
    ...
    # See: https://bitbucket.org/plas/pystart/issues/422/edit-keyboard-shortcuts-ctrl-c-ctrl-v-etc
