...
from pystart.common import (  # TODO: try to get rid of this
...
class BaseBackend(ABC):
    ...
    def _report_internal_exception(self, msg: str) -> None:
        user_msg = "PROBLEM IN PYSTART'S BACK-END: " + msg
        if sys.exc_info()[1]:
            err_msg = "\n".join(
        ...
        user_msg += ".\nSee " + pystart.BACKEND_LOG_MARKER + " for more info."
    ...
    def _handle_normal_command(self, cmd: CommandToBackend) -> None:
        ...
        if handler is None:
            ...
            try:
                ...
                # Exceptions must be caused by PyStart or plugins code, because the ones
...
def get_ssh_password_file_path():
    from pystart import get_pystart_user_dir

    return os.path.join(get_pystart_user_dir(), "ssh_password")
