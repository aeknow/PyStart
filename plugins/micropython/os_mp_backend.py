...
from pystart.backend import SshMixin
from pystart.common import ALL_EXPLAINED_STATUS_CODE, PROCESS_ACK, BackendEvent, serialize_message
from pystart.plugins.micropython.bare_metal_backend import LF, NORMAL_PROMPT
...
logger = getLogger("pystart.plugins.micropython.os_mp_backend")
...
class UnixMicroPythonBackend(MicroPythonBackend, ABC):
    ...
    def _cmd_execute_system_command(self, cmd):
        ...
        self._execute("__pystart_helper.os.system(%r) or None" % cmd_line)
        # TODO: report returncode

    def _cmd_get_fs_info(self, cmd):
        script = """__pystart_helper.os.system("stat -f -c '%b %f %a %S' {path}") or None""".format(
            path=cmd.path
        )
    ...
    def _sync_time(self):
        self._show_error("WARNING: Automatic time synchronization by PyStart is not supported.")

    def _get_utc_timetuple_from_device(self) -> Union[tuple, str]:
        out, err = self._execute("__pystart_helper.os.system('date -u +%s')", capture_output=True)
        if err:
            return err
...
class LocalUnixMicroPythonBackend(UnixMicroPythonBackend):
    def _create_connection(self, run_args=[]):
        from pystart.plugins.micropython.subprocess_connection import SubprocessConnection
...
class SshUnixMicroPythonBackend(UnixMicroPythonBackend, SshMixin):
    ...
    def _create_connection(self, run_args=[]):
        ...
        from pystart.plugins.micropython.ssh_connection import SshProcessConnection
