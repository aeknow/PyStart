...
from pystart import BACKEND_LOG_MARKER, get_backend_log_file, report_time
from pystart.backend import MainBackend
...
from pystart.plugins.micropython.connection import MicroPythonConnection
...
class MicroPythonBackend(MainBackend, ABC):
    ...
    def _prepare_after_soft_reboot(self, clean=False):
        ...
        # See https://github.com/pystart/thonny/issues/1877
        # self._execute_without_output(
        #     dedent(
        #         """
        #     for key in __pystart_helper.builtins.dir(__pystart_helper.builtins):
        #         if not key.startswith("__"):
        #             __pystart_helper.builtins.globals()[key] = None
        #     """
        #     ).strip()
        # )
    ...
    def _perform_gc(self):
        ...
        self._execute_without_output(
            dedent(
                """
            import gc as __pystart_gc
            __pystart_gc.collect()
            del __pystart_gc
        """
            )
        )
            ...
            class __pystart_helper:
                ...
                def repr(cls, obj):
                    try:
                        ...
                        return "<could not serialize: " + __pystart_helper.builtins.str(e) + ">"
    ...
    def _fetch_sys_path(self):
        if not self._supports_directories():
            ...
            return self._evaluate("__pystart_helper.sys.path")
    ...
    def _fetch_epoch_year(self):
        ...
        val = self._evaluate(
            dedent(
                """
            try:
                from time import localtime as __pystart_localtime
                __pystart_helper.print_mgmt_value(__pystart_helper.builtins.tuple(__pystart_localtime(%d)))
                del __pystart_localtime
            except __pystart_helper.builtins.Exception as e:
                __pystart_helper.print_mgmt_value(__pystart_helper.builtins.str(e))
        """
                % Y2000_EPOCH_OFFSET
            )
        )
    ...
    def _update_cwd(self):
        if not self._using_simplified_micropython():
            ...
            self._cwd = self._evaluate("__pystart_helper.getcwd()")
    ...
    def _evaluate(self, script):
        ...
        try:
            ...
            prefix = "__pystart_helper.print_mgmt_value("
            suffix = ")"
    ...
    def _cmd_cd(self, cmd):
        if len(cmd.args) == 1:
            ...
            path = cmd.args[0]
            self._execute("__pystart_helper.chdir(%r)" % path)
            self._update_cwd()
            return {}
        else:
    ...
    def _cmd_get_globals(self, cmd):
        if cmd.module_name == "__main__":
            globs = self._evaluate(
                "{name : (__pystart_helper.repr(value), __pystart_helper.builtins.id(value)) for (name, value) in __pystart_helper.builtins.globals().items() if not name.startswith('__')}"
            )
        else:
            globs = self._evaluate(
                dedent(
                ...
                __pystart_helper.print_mgmt_value(
                    {name : (__pystart_helper.repr(__pystart_helper.builtins.getattr(__mod_for_globs, name)), 
                             __pystart_helper.builtins.id(__pystart_helper.builtins.getattr(__mod_for_globs, name)))
                        in __pystart_helper.builtins.dir(__mod_for_globs) 
    ...
    def _cmd_get_object_info(self, cmd):
        ...
        self._execute(
            dedent(
                """
                if __pystart_helper.builtins.id(__pystart_helper.object_info) not in __pystart_helper.inspector_values:
                    __pystart_helper.inspector_values[__pystart_helper.builtins.id(__pystart_helper.object_info)] = __pystart_helper.object_info
            """
            )
        )
    ...
    def _find_basic_object_info(self, object_id, context_id):
        ...
        to __pystart_helper.object_info.
        ...
        result = self._evaluate(
            dedent(
                """
                for __pystart_helper.object_info in (
                        [__pystart_helper.last_non_none_repl_value]
                        + __pystart_helper.builtins.list(__pystart_helper.builtins.globals().values()) 
                        + __pystart_helper.builtins.list(__pystart_helper.inspector_values.values())):
                    if __pystart_helper.builtins.id(__pystart_helper.object_info) == %d:
                        __pystart_helper.print_mgmt_value({
                            "repr" : __pystart_helper.builtins.repr(__pystart_helper.object_info),
                            "type": __pystart_helper.builtins.str(__pystart_helper.builtins.type(__pystart_helper.object_info))
                        })
                        break
                else:
                    __pystart_helper.object_info = None
                    __pystart_helper.print_mgmt_value(None)
            """
                % object_id
            )
        )
        ...
        if result is not None:
            ...
            return self._evaluate(
                dedent(
                    """
                __pystart_helper.context_value = __pystart_helper.inspector_values.get(%d, None)
                
                if __pystart_helper.context_value is None:
                    __pystart_helper.object_info = None
                    __pystart_helper.print_mgmt_value(None)
                else:
                    __pystart_helper.context_children = [
                         __pystart_helper.builtins.getattr(__pystart_helper.context_value, name)
                         for name in __pystart_helper.builtins.dir(__pystart_helper.context_value)
                    ]
                    if __pystart_helper.builtins.isinstance(__pystart_helper.context_value, (set, tuple, list)):
                        __pystart_helper.context_children += __pystart_helper.builtins.list(__pystart_helper.context_value)
                    elif __pystart_helper.builtins.isinstance(__pystart_helper.context_value, __pystart_helper.builtins.dict):
                        __pystart_helper.context_children += __pystart_helper.builtins.list(__pystart_helper.context_value.values())
                    
                    for __pystart_helper.object_info in __pystart_helper.context_children:
                        if __pystart_helper.builtins.id(__pystart_helper.object_info) == %d:
                            __pystart_helper.print_mgmt_value({
                                "repr" : __pystart_helper.repr(__pystart_helper.object_info),
                                "type": __pystart_helper.builtins.str(__pystart_helper.builtins.type(__pystart_helper.object_info))
                            })
                            break
                    else:
                ...
                __pystart_helper.context_value = None
                __pystart_helper.context_children = None
            """
    ...
    def _get_object_attributes(self, all_attributes):
        """object is given in __pystart_helper.object_info"""
        atts = self._evaluate(
            "{name : ("
            "   __pystart_helper.builtins.id(__pystart_helper.builtins.getattr(__pystart_helper.object_info, name)),"
            "    __pystart_helper.repr(__pystart_helper.builtins.getattr(__pystart_helper.object_info, name))"
            ") for name in __pystart_helper.builtins.dir(__pystart_helper.object_info)}"
        )
        return {
    ...
    def _get_object_info_extras(self, type_name: str, repr_str: str):
        ...
        if type_name in ("list", "tuple", "set"):
            items = self._evaluate(
                "[(__pystart_helper.builtins.id(x), __pystart_helper.repr(x)) for x in __pystart_helper.object_info]"
            )
            return {"elements": [ValueInfo(x[0], x[1]) for x in items]}
        elif type_name == "dict":
            items = self._evaluate(
                "[((__pystart_helper.builtins.id(key), __pystart_helper.repr(key)), (__pystart_helper.builtins.id(__pystart_helper.object_info[key]), "
                "__pystart_helper.repr(__pystart_helper.object_info[key]))) for key in __pystart_helper.object_info]"
            )
            return {
                "entries": [
    ...
    def _mkdir(self, path: str) -> None:
        ...
        self._execute_without_output("__pystart_helper.os.mkdir(%r)" % path)
    ...
    def _delete_sorted_paths(self, paths):
        self._execute_without_output(
            dedent(
                """
            def __pystart_delete(path):
                if __pystart_helper.os.stat(path)[0] & 0o170000 == 0o040000:
                    for name in __pystart_helper.listdir(path):
                        child_path = path + "/" + name
                        __pystart_delete(child_path)
                    __pystart_helper.rmdir(path)
                else:
                    __pystart_helper.os.remove(path)
            
            for __pystart_path in %r: 
                __pystart_delete(__pystart_path)
                
            del __pystart_path
            del __pystart_delete
    ...
    def _get_stat(
        ...
        stat = self._evaluate(
            dedent(
                """
            try:
                __pystart_helper.print_mgmt_value(__pystart_helper.os.%s(%r))
            except __pystart_helper.builtins.Exception:
                __pystart_helper.print_mgmt_value(None)
            """
            )
            % (func, path)
    ...
    def _get_dir_children_info(
        ...
        if self._supports_directories():
            raw_data = self._evaluate(
                ...
                __pystart_result = {} 
                try:
                    __pystart_names = __pystart_helper.listdir(%r)
                except __pystart_helper.builtins.OSError:
                    __pystart_helper.print_mgmt_value(None) 
                else:
                    for __pystart_name in __pystart_names:
                        if not __pystart_name.startswith(".") or %r:
                            try:
                                __pystart_result[__pystart_name] = __pystart_helper.os.stat(%r + __pystart_name)
                            except __pystart_helper.builtins.OSError as e:
                                __pystart_result[__pystart_name] = __pystart_helper.builtins.str(e)
                    __pystart_helper.print_mgmt_value(__pystart_result)
            """
                )
            ...
            raw_data = self._evaluate(
                "{name : __pystart_helper.os.size(name) for name in __pystart_helper.os.listdir()}"
            )
        else:
            return None
    ...
    def _replace_last_repl_value_variables(self, source: str) -> str:
        ...
        for node in reversed(load_nodes):
            lines[node.lineno - 1] = (
                ...
                + "__pystart_helper.builtins.globals().get('_', __pystart_helper.last_non_none_repl_value)"
    ...
    def _add_expression_statement_handlers(self, source):
        try:
            ...
            from pystart.ast_utils import mark_text_ranges
            ...
            marker_prefix = "__pystart_helper.print_repl_value("
            marker_suffix = ")"
    ...
    def _avoid_printing_expression_statements(self, source):
        # temporary solution for https://github.com/pystart/thonny/issues/1441
        try:
            root = ast.parse(source)
