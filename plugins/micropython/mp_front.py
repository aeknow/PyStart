...
from pystart import get_runner, get_shell, get_workbench, running, ui_utils
from pystart.common import CommandToBackend, DistInfo, EOFCommand, ImmediateCommand, InlineCommand
from pystart.config_ui import (
    add_option_checkbox,
    add_option_combobox,
    add_option_entry,
    add_text_row,
...
from pystart.misc_utils import download_and_parse_json
from pystart.plugins.backend_config_page import (
    BaseSshProxyConfigPage,
    TabbedBackendDetailsConfigurationPage,
    get_ssh_password,
)
from pystart.running import SubprocessProxy
...
class MicroPythonProxy(SubprocessProxy):
    ...
    def search_packages(cls, query: str) -> List[DistInfo]:
        from pystart.plugins.pip_gui import compute_dist_name_similarity, perform_pypi_search
...
class BareMetalMicroPythonProxy(MicroPythonProxy):
    def __init__(self, clean):
        ...
        if self._port == "auto":
            # may come from pre-PyStart 5 configuration file
    ...
    def _get_backend_launcher_path(self) -> str:
        import pystart.plugins.micropython.bare_metal_backend

        return pystart.plugins.micropython.bare_metal_backend.__file__
    ...
    def get_switcher_entries(cls) -> List[Tuple[Dict[str, Any], str, str]]:
        def should_show(conf):
            port = conf[f"{cls.backend_name}.port"]
            if port == "auto":
                # may come from pre-PyStart 5 conf
                port = None
    ...
    def open_custom_system_shell(self):
        ...
        terminal.run_in_terminal(
            [
                ...
                "pystart.plugins.micropython.miniterm_wrapper",
                "--exit-char",
                "20",
                "--menu-char",
                "29",
...
class LocalMicroPythonProxy(MicroPythonProxy):
    ...
    def _get_launcher_with_args(self):
        import pystart.plugins.micropython.os_mp_backend

        cmd = [
            pystart.plugins.micropython.os_mp_backend.__file__,
            repr(
                {
