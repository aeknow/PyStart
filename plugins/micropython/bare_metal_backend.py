...
from pystart.backend import UploadDownloadMixin, convert_newlines_if_has_shebang
...
from pystart.misc_utils import find_volumes_by_name
from pystart.plugins.micropython.connection import MicroPythonConnection
from pystart.plugins.micropython.mp_back import (
    EOT,
    NORMAL_MODE_CMD,
    PASTE_MODE_CMD,
...
from pystart.plugins.micropython.mp_common import (
    PASTE_SUBMIT_MODE,
    RAW_PASTE_SUBMIT_MODE,
    RAW_SUBMIT_MODE,
)
from pystart.plugins.micropython.webrepl_connection import WebReplConnection
...
logger = getLogger("pystart.plugins.micropython.bare_metal_backend")
...
class BareMetalMicroPythonBackend(MicroPythonBackend, UploadDownloadMixin):
    ...
    def _check_prepare(self):
        out, err = self._execute(
            "__pystart_helper.builtins.print('__pystart_helper' in __pystart_helper.builtins.dir())",
            capture_output=True,
        )
        if out.strip() == "True":
            return
    ...
    def _get_helper_code(self):
        ...
        if self._read_block_size and self._read_block_size > 0:
            ...
            from pystart.plugins.micropython.serial_connection import OUTPUT_ENQ
            ...
            result += indent(
                ...
                def controlled_print(cls, *args, sep=" ", end="\\n"):
                    for arg_i, arg in enumerate(args):
                        ...
                        data = __pystart_helper.builtins.str(arg)
                        
                        for i in __pystart_helper.builtins.range(0, __pystart_helper.builtins.len(data), cls._print_block_size):
    ...
    def _fetch_builtin_modules(self):
        script = "__pystart_helper.builtins.help('modules')"
    ...
    def _sync_time(self):
        ...
        if self._using_simplified_micropython():
            ...
            specific_script = dedent(
                """
                from rtc import RTC as __pystart_RTC
                __pystart_RTC().datetime = {ts}
                del __pystart_RTC
            """
            ).format(ts=tuple(now))
        else:
            ...
            specific_script = dedent(
                """
                from machine import RTC as __pystart_RTC
                try:
                    __pystart_RTC().datetime({datetime_ts})
                except:
                    __pystart_RTC().init({init_ts})
                finally:
                    del __pystart_RTC
        ...
        script = (
            ...
            try:
            %s
                __pystart_helper.print_mgmt_value(True)
    ...
    def _get_utc_timetuple_from_device(self) -> Union[tuple, str]:
        if self._using_simplified_micropython():
            ...
            specific_script = dedent(
                """
                from rtc import RTC as __pystart_RTC
                __pystart_helper.print_mgmt_value(__pystart_helper.builtins.tuple(__pystart_RTC().datetime)[:6])
                del __pystart_RTC
                """
            )
        else:
            specific_script = dedent(
                ...
                try:
                    ...
                    __pystart_temp = __pystart_helper.builtins.tuple(__pystart_RTC().datetime())
                    # remove weekday from index 3
                    __pystart_helper.print_mgmt_value(__pystart_temp[0:3] + __pystart_temp[4:7])
                    del __pystart_temp
                except:
                    __pystart_helper.print_mgmt_value(__pystart_helper.builtins.tuple(__pystart_RTC().now())[:6])
                del __pystart_RTC
                """
            )
        ...
        script = dedent(
            ...
            try:
                try:
                    ...
                    __pystart_helper.print_mgmt_value(__pystart_helper.builtins.tuple(__pystart_localtime()))
                    del __pystart_localtime
                except:
                    # some CP boards
                    from rtc import RTC as __pystart_RTC
                    __pystart_helper.print_mgmt_value(__pystart_helper.builtins.tuple(__pystart_RTC().datetime))
                    del __pystart_RTC
            except __pystart_helper.builtins.Exception as e:
                __pystart_helper.print_mgmt_value(__pystart_helper.builtins.str(e))
        """
    ...
    def _clear_repl(self):
        """NB! assumes prompt and may be called without __pystart_helper"""
    ...
    def _process_output_until_active_prompt(
        ...
        Note that this Ctrl+A may have been issued even before PyStart connected to
        the device.
        ...
        output or the starting EOT from PyStart expression (or, in principle, even
        the second raw-REPL EOT or terminating PyStart expression EOT)
        ...
        if cmd.get("source"):
            ...
            if cmd.get("populate_argv", False):
                ...
                argv_updater = textwrap.dedent(
                    f"""
                try:
                    import sys as _pystart_sys
                    _pystart_sys.argv[:] = {argv}
                    del __pystart_sys
                except:
                    pass
                """
    ...
    def _cmd_get_fs_info(self, cmd):
        if self._using_simplified_micropython():
            used = self._evaluate(
                dedent(
                    ...
                    __pystart_helper.print_mgmt_value(
                        __pystart_helper.builtins.sum([__pystart_helper.os.size(name) for name in __pystart_helper.os.listdir()])
                    )  
                    """
                )
            )
            ...
            result = self._evaluate(
                dedent(
                    """
                __pystart_stat = __pystart_helper.os.statvfs(%r)
                __pystart_total = __pystart_stat[2] * __pystart_stat[0]
                __pystart_free = __pystart_stat[3] * __pystart_stat[0]

                __pystart_helper.print_mgmt_value({
                    "total" : __pystart_total,
                    "used" : __pystart_total - __pystart_free,
                    "free": __pystart_free,
                })  
        ...
        if not self._supports_directories():
            ...
            self._execute_without_output(
                ...
                for __pystart_path in %r: 
                    __pystart_helper.os.remove(__pystart_path)
    ...
    def _read_file_via_serial(
        ...
        self._execute_without_output(
            "__pystart_fp = __pystart_helper.builtins.open(%r, 'rb')" % source_path
        )
        if hex_mode:
        ...
        while True:
            ...
            if hex_mode:
                block = binascii.unhexlify(
                    self._evaluate("__temp_hexlify(__pystart_fp.read(%s))" % block_size)
                )
            else:
                block = self._evaluate("__pystart_fp.read(%s)" % block_size)
    ...
    def _write_file_via_serial(
        ...
        out, err = self._execute(
            dedent(
                ...
                __pystart_path = '{path}'
                __pystart_written = 0
                __pystart_fp = __pystart_helper.builtins.open(__pystart_path, 'wb')
            """
        ...
        if hex_mode:
            self._execute_without_output(
                dedent(
                    """
                from binascii import unhexlify as __pystart_unhex
                def __W(x):
                    global __pystart_written
                    __pystart_written += __pystart_fp.write(__pystart_unhex(x))
                    __pystart_fp.flush()
                    if __pystart_helper.builtins.hasattr(__pystart_helper.os, "sync"):
                        __pystart_helper.os.sync()
            """
                )
            )
            ...
            self._execute_without_output(
                ...
                def __W(x):
                    global __pystart_written
                    __pystart_written += __pystart_fp.write(x)
            """
                )
            )
        else:
        ...
        bytes_received = self._evaluate("__pystart_written")
        ...
        self._execute_without_output(
            dedent(
                ...
                try:
                    del __W
                    del __pystart_written
                    del __pystart_path
                    __pystart_fp.close()
                    del __pystart_fp
                    del __pystart_result
                    del __pystart_unhex
                except:
                    pass
            """
    ...
    def _get_fs_mount_label(self):
        ...
        result = self._evaluate(
            dedent(
                """
            try:
                from storage import getmount as __pystart_getmount
                try:
                    __pystart_result = __pystart_getmount("/").label
                finally:
                    del __pystart_getmount
            except __pystart_helper.builtins.ImportError:
                __pystart_result = None 
...
def launch_bare_metal_backend(backend_class: Callable[..., BareMetalMicroPythonBackend]) -> None:
    ...
    try:
        if args["port"] is None:
            ...
            from pystart.plugins.micropython.serial_connection import (
