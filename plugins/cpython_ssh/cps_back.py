...
logger = getLogger("pystart.plugins.cpython_ssh.cps_back")
...
class SshCPythonBackend(BaseBackend, SshMixin):
    ...
    def _start_main_backend(self) -> RemoteProcess:
        env = {"PYSTART_USER_DIR": "~/.config/PyStart", "THONNY_FRONTEND_SYS_PATH": "[]"}
        self._main_backend_is_fresh = True
    ...
    def _get_remote_program_directory(self):
        return f"/tmp/thonny-backend-{pystart.get_version()}-{self._user}"
    ...
    def _upload_main_backend(self):
        ...
        if self._get_stat_mode_for_upload(launch_dir) and not pystart.get_version().endswith("-dev"):
            # don't overwrite unless in dev mode
            return

        ensure_posix_directory(
            launch_dir + "/pystart/plugins/cpython_backend",
            self._get_stat_mode_for_upload,
            self._mkdir_for_upload,
        )

        import pystart.ast_utils
        import pystart.backend
        import pystart.plugins.cpython_backend.cp_back

        # Don't want to import cp_back_launcher and cp_tracers

        local_context = os.path.dirname(os.path.dirname(pystart.__file__))
        for local_path in [
            pystart.__file__,
            pystart.common.__file__,
            pystart.ast_utils.__file__,
            pystart.backend.__file__,
            pystart.backend.__file__.replace("backend.py", "VERSION"),
            pystart.plugins.cpython_backend.__file__,
            pystart.plugins.cpython_backend.cp_back.__file__,
            pystart.plugins.cpython_backend.cp_back.__file__.replace("cp_back.py", "cp_launcher.py"),
            pystart.plugins.cpython_backend.cp_back.__file__.replace("cp_back.py", "cp_tracers.py"),
        ]:
