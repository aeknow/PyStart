...
In this mode you need to notify PyStart that you're ready to let Python make the next step. 
...
you can **take back the step** by selecting *Run → Step back*. PyStart will show
the state of program as it was before the last step. Now you can continue with small steps
and zoom into this piece of code. (How does it work? Even when you take a big step, PyStart
...
This makes PyStart automatically step until this line. You can take the command from there.
...
-------------------------------------------
By default PyStart uses stacked windows for presenting the call stack. This gives good intuition about 
