...
Birdseye works differently than `PyStart's built-in debuggers <debuggers.rst>`_. 
...
But when the program completes, PyStart opens a webpage (served by a local server provided 
by Birdseye), which allows you to dig into the execution process and learn how final results were composed
of intermediate values. 

NB! When using Birdseye in PyStart you don't need to import ``birdseye.eye`` or use it 
for decorating your functions. PyStart executes Birdseye such that it records information about all
functions.

The local server uses port 7777 by default. If this is used by another application, then configure
another port in *Tools → Options → Run & Debug* and restart PyStart.
