...
from pystart.plugins.micropython.bare_metal_backend import (
    BareMetalMicroPythonBackend,
    launch_bare_metal_backend,
)

# Can't use __name__, because it will be "__main__"
logger = getLogger("pystart.plugins.micropython.cirpy_backend")
...
class CircuitPythonBackend(BareMetalMicroPythonBackend):
    def _fetch_board_id(self) -> Optional[str]:
        ...
        try:
            ...
            __pystart_helper.print_mgmt_value(__temp_board_id)
            del __temp_board_id
        except ImportError:
    ...
    def _transform_output(self, data, stream_name):
        # Any keypress wouldn't work in PyStart's shell
