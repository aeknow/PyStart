...
from pystart.backend import MainBackend, logger
...
_REPL_HELPER_NAME = "_pystart_repl_print"

_CONFIG_FILENAME = os.path.join(pystart.get_pystart_user_dir(), "backend_configuration.ini")
...
class MainCPythonBackend(MainBackend):
    def __init__(self, target_cwd, options):
        ...
        self._main_dir = os.path.dirname(sys.modules["pystart"].__file__)
    ...
    def _init_command_reader(self):
        ...
        # https://github.com/pystart/thonny/issues/1363
        pass
    ...
    def _handle_immediate_command(self, cmd: ImmediateCommand) -> None:
        ...
        was reverted for CPython https://github.com/pystart/thonny/issues/1363.
    ...
    def _load_plugins(self):
        # built-in plugins
        try:
            import pystart.plugins.backend  # pylint: disable=redefined-outer-name
        except ImportError:
            # May happen eg. in ssh session
            return

        self._load_plugins_from_path(pystart.plugins.backend.__path__, "pystart.plugins.backend.")

        # 3rd party plugins from namespace package
        try:
            import pystartcontrib.backend  # @UnresolvedImport
        except ImportError:
            # No 3rd party plugins installed
            pass
        else:
            self._load_plugins_from_path(pystartcontrib.backend.__path__, "pystartcontrib.backend.")
    ...
    def _cmd_get_environment_info(self, cmd):
        return ToplevelResponse(
            ...
            cwd=os.getcwd(),
            logfile=pystart.get_backend_log_file(),
        )
    ...
    def _cmd_FastDebug(self, cmd):
        self.switch_env_to_script_mode(cmd)
        from pystart.plugins.cpython_backend.cp_tracers import FastTracer
    ...
    def _cmd_Debug(self, cmd):
        ...
        from pystart.plugins.cpython_backend.cp_tracers import NiceTracer
    ...
    def export_value(self, value, max_repr_length=5000):
        ...
        try:
            ...
            # See https://bitbucket.org/plas/pystart/issues/584/problem-with-thonnys-back-end-obj-no
            rep = "??? <repr error>"
...
def in_debug_mode():
    return os.environ.get("PYSTART_DEBUG", False) in [1, "1", True, "True", "true"]
