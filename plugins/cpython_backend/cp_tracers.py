...
from pystart.plugins.cpython_backend.cp_back import Executor, format_exception_with_frame_info

BEFORE_STATEMENT_MARKER = "_pystart_hidden_before_stmt"
BEFORE_EXPRESSION_MARKER = "_pystart_hidden_before_expr"
AFTER_STATEMENT_MARKER = "_pystart_hidden_after_stmt"
AFTER_EXPRESSION_MARKER = "_pystart_hidden_after_expr"
...
class Tracer(Executor):
    def __init__(self, backend, original_cmd):
        ...
        self._pystart_src_dir = os.path.dirname(sys.modules["pystart"].__file__)
    ...
    def _is_interesting_module_file(self, path):
        ...
        result = (
            ...
            and not path_startswith(path, self._pystart_src_dir)
        )
    ...
    def _register_affected_frame(self, exception_obj, frame):
        ...
        # but Python 3.8 doesn't allow this (https://github.com/pystart/thonny/issues/1403)
...
class NiceTracer(Tracer):
    ...
    def _pystart_hidden_before_stmt(self, node_id):
    ...
    def _pystart_hidden_after_stmt(self, node_id):
        # The code to be debugged will be instrumented with this function
    ...
    def _pystart_hidden_before_expr(self, node_id):
        # Entry into this function indicates that expression as given
        # by the code range is about to be evaluated next
        return node_id

    def _pystart_hidden_after_expr(self, node_id, value):
        # The code to be debugged will be instrumented with this function
    ...
    def _should_instrument_as_statement(self, node):
        return (
            ...
            # https://bitbucket.org/plas/pystart/issues/183/thonny-throws-false-positive-syntaxerror
    ...
    def _execute_prepared_user_code(self, statements, global_vars):
        try:
            ...
            """
            from pystart.misc_utils import _win_get_used_memory
