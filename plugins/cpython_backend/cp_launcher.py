...
if __name__ == "__main__":
    ...
    if sys.platform == "darwin":
        try:
            ...
            print(
                "\nNB! Potential problems detected, see\nhttps://github.com/pystart/thonny/wiki/MacOSX#catalina\n",
                file=sys.stderr,
            )

    if not sys.version_info > (3, 9):
        print(
            "This version of PyStart only supports Python 3.9 and later.\n"
    ...
    pystart.PYSTART_USER_DIR = pystart.get_pystart_user_dir()

    from pystart import report_time

    report_time("Before importing MainCPythonBackend")
    from pystart.common import PROCESS_ACK
    from pystart.plugins.cpython_backend.cp_back import MainCPythonBackend

    pystart.prepare_pystart_user_dir()  # required when run on remote machine over SSH
