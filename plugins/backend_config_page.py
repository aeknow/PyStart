...
from pystart.backend import delete_stored_ssh_password, get_ssh_password_file_path
...
from pystart.running import BackendProxy
from pystart.ui_utils import CommonDialogEx, create_string_var, ems_to_pixels
...
class TabbedBackendDetailsConfigurationPage(BackendDetailsConfigPage):
    ...
    def create_and_add_stubs_page(self, proxy_class: type[BackendProxy]) -> ttk.Frame:
        from pystart.plugins.pip_gui import StubsPipFrame
...
class BackendConfigurationPage(ConfigurationPage):
    def __init__(self, master):
        ...
        label = ttk.Label(
            self, text=tr("Which kind of interpreter should PyStart use for running your code?")
        )
