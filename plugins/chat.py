...
from pystart import get_runner, get_shell, get_workbench, rst_utils, tktextext, ui_utils
from pystart.assistance import (
    Assistant,
    Attachment,
    ChatContext,
    ChatMessage,
...
from pystart.common import STRING_PSEUDO_FILENAME, ToplevelResponse
from pystart.languages import tr
from pystart.tktextext import EnhancedText, TweakableText
...
class ChatView(tktextext.TextFrame):
    def __init__(self, master):
        ...
        # Start with OpenAI assistant if available, otherwise EchoAssistant
        if "openai" in get_workbench().assistants:
            self._current_assistant: Assistant = get_workbench().assistants["openai"]
        else:
    ...
    def _workspace_ready(self, event: tk.Event) -> None:
        # Keep current assistant (already set to OpenAI if available)
        self._update_suggestions()
    ...
    def compile_attachments(self, message: str) -> Tuple[List[Attachment], List[str]]:
        from pystart.shell import ExecutionInfo
        ...
        # Check if clearText tag is present and clear the chat window
        if "clearText" in tags:
            self._clear_chat_text()
            # Return early if only clearText tag is present
            if len(tags) == 1:
                return [], []
                ...
                if current_editor is not None:
                    if current_editor.is_local():
                        try:
                            ...
                            # Handle case where file and cwd are on different drives
                            editor_name = current_editor.get_target_path()
    ...
    def _update_suggestions(self) -> None:
        ...
        if editor is not None and editor.get_content():
            new_suggestions.append(tr("检查 #当前文件"))

        if get_shell().text.has_selection() and last_run_info is not None:
            new_suggestions.append(tr("Explain #selectedOutput in #lastRun"))

        if last_run_info is not None or True:
            new_suggestions.append(tr("解释 #运行信息"))
            
        # Always add clear text option
        new_suggestions.append(tr(" #清空聊天"))
    ...
    def _clear_chat_text(self) -> None:
        """Clear the chat text content"""
        ...
        # Reset analysis indices
        self._last_analysis_start_index = "1.0"
        self._last_analysis_end_index = "1.0"
        
        # Clear formatted attachments
        self._formatted_attachmets_per_message.clear()
        
        # Clear last tagged attachments
        self._last_tagged_attachments.clear()
        
        # Reset active chat request ID
        self._active_chat_request_id = None
        
        logger.info("Chat text cleared")
