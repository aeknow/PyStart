...
import pystart
from pystart import SUPPORTED_VERSIONS, get_runner, get_shell, get_workbench, ui_utils
...
from pystart.misc_utils import running_on_mac_os, running_on_windows
from pystart.plugins.backend_config_page import TabbedBackendDetailsConfigurationPage
from pystart.running import WINDOWS_EXE, SubprocessProxy
from pystart.terminal import run_in_terminal
from pystart.ui_utils import askopenfilename, create_string_var, ems_to_pixels
...
class LocalCPythonProxy(SubprocessProxy):
    ...
    def _get_launch_cwd(self):
        ...
        empty_dir = os.path.join(pystart.get_pystart_user_dir(), "leave_this_empty")
        ...
        if force or get_runner().is_waiting_toplevel_command():
            ...
            if not self._expecting_response_for_gui_update:
                try:
                    ...
                    # https://github.com/pystart/thonny/issues/966
...
class LocalCPythonConfigurationPage(TabbedBackendDetailsConfigurationPage):
    def __init__(self, master):
        ...
        extra_text = tr("NB! PyStart only supports Python %s and later") % SUPPORTED_VERSIONS[0]
        ...
        file_browser_hint = ttk.Label(
            ...
            text=tr(
                ...
                "right-click menu in PyStart's file browser."
            ),
        )
...
def _get_interpreters_from_windows_registry():
    ...
    for key in [winreg.HKEY_LOCAL_MACHINE, winreg.HKEY_CURRENT_USER]:
        for version in pystart.SUPPORTED_VERSIONS:
            for suffix in ["", "-32", "-64"]:
    ...
    if running_on_windows():
        ...
        for dir_ in dirs:
            ...
            if apath != dir_ and apath in dirs:
                continue
            for version in ["3"] + pystart.SUPPORTED_VERSIONS:
                name = "python" + version
