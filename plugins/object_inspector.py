...
import pystart.memory
from pystart import get_runner, get_workbench, ui_utils
from pystart.common import InlineCommand
from pystart.languages import tr
from pystart.memory import MemoryFrame
from pystart.misc_utils import shorten_repr
from pystart.tktextext import TextFrame
from pystart.ui_utils import CustomToolbutton, ems_to_pixels
...
class ObjectInspector(ttk.Frame):
    ...
    def _show_object_by_id(self, object_id, via_navigation=False):
        ...
        if self.winfo_ismapped() and self.object_id != object_id:
            ...
            self._set_title("object @ " + pystart.memory.format_object_id(object_id))
    ...
    def set_object_info(self, object_info):
        ...
        if object_info is None or "error" in object_info:
            ...
            self._set_title(
                ...
                + " @ "
                + pystart.memory.format_object_id(object_info["id"])
            )
...
class ElementsInspector(pystart.memory.MemoryFrame, ContentInspector):
    def __init__(self, master):
        ContentInspector.__init__(self, master)
        pystart.memory.MemoryFrame.__init__(
    ...
    def set_object_info(self, object_info):
        ...
        for element in object_info["elements"]:
            ...
            self.tree.set(node_id, "id", pystart.memory.format_object_id(element.id))
            self.tree.set(
                node_id, "value", shorten_repr(element.repr, pystart.memory.MAX_REPR_LENGTH_IN_GRID)
            )
            index += 1
...
class DictInspector(pystart.memory.MemoryFrame, ContentInspector):
    def __init__(self, master):
        ContentInspector.__init__(self, master)
        pystart.memory.MemoryFrame.__init__(
    ...
    def set_object_info(self, object_info):
        ...
        for key, value in object_info["entries"]:
            node_id = self.tree.insert("", "end")
            self.tree.set(node_id, "key_id", pystart.memory.format_object_id(key.id))
            self.tree.set(
                node_id, "key", shorten_repr(key.repr, pystart.memory.MAX_REPR_LENGTH_IN_GRID)
            )
            self.tree.set(node_id, "id", pystart.memory.format_object_id(value.id))
            self.tree.set(
                node_id, "value", shorten_repr(value.repr, pystart.memory.MAX_REPR_LENGTH_IN_GRID)
            )
...
class AttributesFrame(pystart.memory.VariablesFrame):
    def __init__(self, master):
        pystart.memory.VariablesFrame.__init__(self, master, consider_heading_stripe=False)
    ...
    def show_selected_object_info(self):
        ...
        if repr_str == "<bound_method>":
            from pystart.plugins.micropython import MicroPythonProxy
