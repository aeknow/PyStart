...
import pystart
from pystart import get_runner, get_workbench, running, tktextext, ui_utils
...
from pystart.running import BackendProxy, InlineCommandDialog, get_front_interpreter_for_subprocess
...
from pystart.workdlg import SubprocessDialog
    ...
    def _complete_show_package_info(self, dist_info: DistInfo):
        ...
        if dist_info.requires:
            ...
            for item in dist_info.requires:
                ...
                # avoid introducing a new dependency as PyStart is included in
...
class BackendPipFrame(PipFrame):
    ...
    def _show_extra_instructions(self):
        from pystart.plugins.micropython.mp_front import MicroPythonProxy
...
class PluginsPipFrame(PipFrame):
    ...
    def _conflicts_with_pystart_version(self, req_strings):
        try:
            conflicts = []
            for req_string in req_strings:
                req = Requirement(req_string)
                if req.name == "pystart" and pystart.get_version() not in req:
    ...
    def _confirm_install(self, package_data):
        if not self._looks_like_plug_in(package_data) and not messagebox.askyesno(
            tr("Confirmation"),
            tr(
                "This doesn't look like PyStart plug-in.\n\n"
                "If you want to install it for your programs, then use\n"
                "'Tools => Manage packages' instead.\n\n"
                "Are you sure you want to install it as PyStart plug-in?"
            ),
            master=self,
        ):
            return False
        ...
        other_version_text = tr(
            ...
            + "which is compatible with current PyStart version. "
            + "Click on '...' button to choose the version to install."
        )

        if name.lower().startswith("thonny-") and not reqs:
            showerror(
                tr("PyStart plugin without requirements"),
                (
                    "Looks like you are trying to install an outdated PyStart\n"
                    + "plug-in (it doesn't specify required PyStart version\n"
            ...
            conflicts = self._conflicts_with_pystart_version(reqs)
            if conflicts:
                showerror(
                    tr("Unsuitable requirements"),
                    tr("This package requires different PyStart version:")
                    + "\n\n  "
                    + "\n  ".join(conflicts)
                    + "\n\n"
    ...
    def _get_target_directory(self):
        if running_in_virtual_environment():
            ...
            target = pystart.get_sys_path_directory_containg_plugins()
    ...
    def _run_pip_with_dialog(self, command: str, args: Dict, title: str) -> Tuple[int, str, str]:
        ...
        proc = running.create_frontend_python_process(
            ...
            environment_extras={"PYTHONUSERBASE": pystart.get_user_base_directory_for_plugins()},
        )
    ...
    def _fetch_search_results(self, query: str) -> List[DistInfo]:
        return perform_pypi_search(
            query, get_workbench().get_data_url("pypi_summaries_cpython.json"), ["pystart"]
        )
...
class PluginsPipDialog(CommonDialog):
    def __init__(self, master):
        ...
        banner_msg = (
            tr(
                "This dialog is for managing PyStart plug-ins and their dependencies.\n"
        ...
        banner_msg += "\n" + tr(
            "NB! You need to restart PyStart after installing / upgrading / uninstalling a plug-in."
        )
        ...
        self.title(tr("PyStart plug-ins"))
...
class StubsPipFrame(PipFrame):
    ...
    def _run_pip_with_dialog(self, command: str, args: Dict, title: str) -> Tuple[int, str, str]:
        ...
        proc = running.create_frontend_python_process(
            cmd,
            stderr=subprocess.STDOUT,
            environment_extras={"PYTHONPATH": pystart.get_vendored_libs_dir()},
        )
        dlg = SubprocessDialog(
            self,
            prepared_proc=proc,
