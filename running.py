...
class BackendProxy(ABC):
    ...
    def get_package_installation_confirmations(self, dist_info: DistInfo) -> List[str]:
        if "pystart" in dist_info.name.lower():
            return [
                tr(
                    "Looks like you are installing a PyStart-related package.\n"
                    + "If you meant to install a PyStart plugin, then you should\n"
    ...
    def search_packages(cls, query: str) -> List[DistInfo]:
        from pystart.plugins.pip_gui import perform_pypi_search
    ...
    def get_package_info_from_index(cls, name: str, version: str) -> DistInfo:
        from pystart.plugins.pip_gui import download_dist_info_from_pypi

        return download_dist_info_from_pypi(name, version)

    @classmethod
    def get_version_list_from_index(cls, name: str) -> List[str]:
        from pystart.plugins.pip_gui import try_download_version_list_from_pypi
    ...
    def get_user_stubs_location(cls) -> str:
        result_path = os.path.join(pystart.get_pystart_user_dir(), "stubs", cls.backend_name)
...
class SubprocessProxy(BackendProxy, ABC):
    ...
    def _get_environment(self):
        ...
        # see https://github.com/pystart/thonny/issues/808
        env["PYTHONUNBUFFERED"] = "1"

        env["PYTHONSAFEPATH"] = "1"

        # Let back-end know about plug-ins
        env["PYSTART_USER_DIR"] = get_pystart_user_dir()
        ...
        if pystart.in_debug_mode():
            env["PYSTART_DEBUG"] = "1"
        elif "PYSTART_DEBUG" in env:
            del env["PYSTART_DEBUG"]
        return env
    ...
    def _listen_stderr(self, stderr):
        while True:
            ...
            if data == "":
                ...
                # Filter out Python portable installation warning
                if "Could not find platform independent libraries" in data:
                    logger.debug("Filtered stderr: %r", data)
                    continue
...
def get_environment_overrides_for_python_subprocess(target_executable):
    ...
    # exe is bundled for PyStart.
    # In remaining cases it is user's responsibility to avoid
    # calling PyStart with environment which may be confusing for
    ...
    if os.path.samefile(
        ...
        # (If they are set then it's most likely because current exe is in PyStart bundle)
