...
from pystart import dnd  # inspired by and very similar to tkinter.dnd
...
class CustomNotebook(tk.Frame):
    ...
    def select_another_tab(self, event: tk.Event) -> Optional[str]:
        ...
        from pystart.ui_utils import shift_is_pressed
...
def _ems_to_pixels(x: float) -> int:
    from pystart.ui_utils import ems_to_pixels

    return ems_to_pixels(x)


def _get_style_configuration(name: str) -> Dict:
    from pystart.ui_utils import get_style_configuration
