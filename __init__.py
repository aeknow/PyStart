...
if TYPE_CHECKING:
    ...
    from pystart.running import Runner
    from pystart.shell import ShellView
    from pystart.workbench import Workbench
...
BACKEND_LOG_MARKER = "PyStart's backend.log"
REPORT_TIME = False
...
def get_version():
    ...
    if _version:
        return _version
    try:
        package_dir = os.path.dirname(sys.modules["pystart"].__file__)
        with open(os.path.join(package_dir, "VERSION"), encoding="ASCII") as fp:
            _version = fp.read().strip()
            return _version
    except Exception:
        logger.warning("Could not determine PyStart version", exc_info=True)
        return "0.0.0"
...
def get_ipc_file_path():
    ...
    if sys.platform == "win32":
        from pystart.misc_utils import get_local_appdata_dir
    ...
    ipc_dir = os.path.join(base_dir, "pystart-%s" % username)
...
def in_debug_mode() -> bool:
    ...
    if _in_debug_mode is None:
        _in_debug_mode = (
            os.environ.get("PYSTART_DEBUG", False)
            in [
                "1",
                1,
                "True",
...
def is_portable() -> bool:
    ...
    portable_marker_path = os.path.join(os.path.dirname(sys.executable), "portable_pystart.ini")
    shared_marker_path = os.path.join(os.path.dirname(sys.executable), "shared_pystart.ini")
...
def get_pystart_user_dir() -> str:
    global _pystart_user_dir
    if _pystart_user_dir is None:
        _pystart_user_dir = _compute_pystart_user_dir()
    return _pystart_user_dir


def get_configuration_file() -> str:
    global _configuration_file
    if _configuration_file is None:
        _configuration_file = os.path.join(get_pystart_user_dir(), "pystart.ini")

    return _configuration_file


def prepare_pystart_user_dir():
    if not os.path.exists(get_pystart_user_dir()):
        os.makedirs(get_pystart_user_dir(), mode=0o700, exist_ok=True)
        ...
        if os.path.isdir(template_dir):
            ...
            copy_contents(template_dir, get_pystart_user_dir())
...
def set_logging_level(level=None):
    ...
    logging.getLogger("pystart").setLevel(level)
...
def get_backend_log_file():
    ...
    return os.path.join(get_pystart_user_dir(), file_name)
...
def configure_logging(log_file, console_level=None):
    ...
    main_logger = logging.getLogger("pystart")
    contrib_logger = logging.getLogger("pystartcontrib")
    ...
    main_logger.info("PyStart version: %s", get_version())
    ...
    fault_out = open(
        os.path.join(get_pystart_user_dir(), "frontend_faults.log"), mode="w", buffering=1
    )
...
def get_user_base_directory_for_plugins() -> str:
    return os.path.join(get_pystart_user_dir(), "plugins")


def get_sys_path_directory_containg_plugins() -> str:
    from pystart.misc_utils import get_user_site_packages_dir_for_base
...
def _compute_pystart_user_dir():
    env_var = os.environ.get("PYSTART_USER_DIR", "")
    if env_var:
        # back-end processes always choose this path
        return os.path.expanduser(env_var)

    return os.path.join(_compute_pystart_profiles_dir(), get_profile())


def _compute_pystart_profiles_dir():
    # Following is used only in the front-end process
    from pystart.common import is_private_python, running_in_virtual_environment
    from pystart.misc_utils import get_roaming_appdata_dir
    ...
    if is_portable():
        ...
        return os.path.join(sys.prefix, ".pystart")
    elif sys.platform == "win32":
        return os.path.join(get_roaming_appdata_dir(), "PyStart")
    elif sys.platform == "darwin":
        return os.path.expanduser("~/Library/PyStart")
    else:
        # https://specifications.freedesktop.org/basedir-spec/latest/ar01s02.html
        data_home = os.environ.get(
            "XDG_CONFIG_HOME", os.path.expanduser(os.path.join("~", ".config"))
        )
        return os.path.join(data_home, "PyStart")
...
def _get_orig_argv() -> Optional[List[str]]:
    try:
        ...
        try:
            ...
            # See https://github.com/pystart/thonny/issues/2206
            # and https://bugs.python.org/issue40910
            # This symbol is not available in pystart.exe built against Python 3.8
            return None
