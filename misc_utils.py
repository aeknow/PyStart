...
from pystart import get_runner
...
def list_volumes(skip_letters=set()) -> Sequence[str]:
    ...
    if sys.platform == "linux":
        ...
        from pystart.udisks import list_volumes_sync
...
def find_volume_by_name(
    ...
    if len(volumes) == 1:
        ...
        from pystart.ui_utils import askdirectory
...
def show_command_not_available_in_flatpak_message():
    ...
    messagebox.showinfo(
        ...
        tr("This command is not available if PyStart is run via Flatpak"),
...
def uri_to_legacy_filename(uri: str) -> str:
    """Returns 'blah :: /path/file' for remote files, as was common in PyStart 4"""
...
def local_path_to_uri(path: str) -> str:
    ...
    elif path[1:3] == ":\\" or path[1:3] == ":/":
        # Regular Windows path (with backslash or forward slash), needs special treatment on other platforms
        # Normalize to use backslashes for PureWindowsPath
