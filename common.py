...
def serialize_message(msg: Record, max_line_length=65536) -> str:
    ...
    # default (safe) window size in Paramiko (https://github.com/pystart/thonny/issues/1680)
    msg_str = ascii(msg)
    ...
    if os.name == "nt":
        try:
            ...
            if result.casefold() != norm_name.casefold():
                ...
                # see eg. https://github.com/pystart/thonny/issues/925
        ...
        # Hopefully only correct case comes into PyStart (eg. via open dialog)
        return os.path.normpath(name)
...
def get_windows_lnk_target(lnk_file_path):
    ...
    script_path = os.path.join(os.path.dirname(pystart.__file__), "res", "PrintLnkTarget.vbs")
