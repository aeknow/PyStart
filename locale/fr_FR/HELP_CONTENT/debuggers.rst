...
Dans ce mode il faut notifier PyStart qu'on est prêt à laisser Python faire le pas suivant.
...
on peut **défaire cette étape** en sélectionnant *Lancer → Un pas en arrière*. PyStart montrera l'état du programme tel qu'il était avant le dernier pas. Ensuite on peut continuer avec de petits pas
et zoomer ce morceau de code. (Comment ça marche ? Même quand on réalise un grand pas, PyStart
...
Cela fait avancer PyStart automatiquement en mode pas-à-pas jusqu'à cette ligne. Vous pouvez reprendre la main à partir de là.
...
Par défaut PyStart utilise des fenêtres empilées pour représenter la pile d'appels. Cela donne une bonne intuition du
