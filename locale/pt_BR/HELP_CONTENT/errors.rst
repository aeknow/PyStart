...
As mensagens de erro no PyStart têm links que o levam ao local no código que causou o erro. No caso de vários links, o último é geralmente o mais relevante.
...
A última linha do bloco de erro diz qual foi o problema do Python. Ao tentar compreender a mensagem, não se esqueça do contexto e tente combinar algumas partes da mensagem com o local vinculado ao código. Às vezes o Assistente do PyStart pode explicar o erro em termos mais simples, às vezes você precisa fazer uma busca na Internet pela mensagem (não se esqueça de adicionar "Python" à busca).
...
Veja `a página sobre depuração <debugging.rst>`_ ou `a página sobre como usar os depuradores do PyStart <debuggers.rst>`_.
