...
Assim como o REPL oficial do Python, o Shell do PyStart aceita expressões e instruções de Python, tanto de uma linha quanto com várias linhas. Se você pressionar o ENTER, PyStart usará algumas heurísticas para prever se você deseja enviar o comando ou continuar digitando o restante do comando na próxima linha. Se você deseja enviar o comando, mas PyStart oferece uma nova linha, verifique se você esqueceu de fechar alguns parênteses.

Comandos mágicos
----------------

Se você selecionar *Executar →  Executar programa atual* ou pressionar F5, verá como o PyStart insere um comando começando com ``%Run`` no Shell. Comandos começando com ``%`` são chamados de *comandos mágicos* (assim como em `IPython <https://ipython.org/>`_) e executam certas ações, que não podem ser (facilmente) expressas como comandos do Python. Os comandos mágicos do PyStart geralmente têm comandos de menu correspondentes, então você não precisa escrevê-los manualmente.

Comandos do sistema
-------------------

Se você precisar executar rapidamente um comando simples do sistema, não precisará iniciar um terminal. Basta prefixar o comando com ``!`` (por exemplo, ``!pytest meu-programa.py``) e inseri-lo no Shell do PyStart.
...
Emuladores de terminal apropriados suportam códigos ANSI que permitem escrever em posições arbitrárias na tela do terminal. O Shell do PyStart não é tão capaz, mas suporta alguns truques mais simples.
