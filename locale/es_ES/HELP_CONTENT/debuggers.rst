...
En este modo necesita notificar a PyStart que está listo para dejar que Python haga el siguiente paso. 
...
*Ejecutar → Entrando* (o F7) intenta dar pasos más pequeños. Si el código resaltado está hecho de partes más pequeñas (declaraciones o expresiones), entonces la primera de ellas se resalta y PyStart espera el siguiente comando. Si ha llegado a un componente del programa que no tiene subpartes (por ejemplo, un nombre de variable) entonces *Paso a* funciona como *Paso a*, es decir, ejecuta (o evalúa) el código.
...
PyStart mostrará el estado del programa tal y como estaba antes del último paso.
Ahora puede continuar con pequeños pasos y hacer zoom en este trozo de código.
(¿Cómo funciona? Incluso cuando usted da un gran paso, PyStart guarda todos los estados intermedios del programa, que puede reproducir después de dar el paso atrás). 
...
Esto hace que PyStart se desplace automáticamente hasta esa línea. A partir de ahí puede tomar el comando.
...
Por defecto, PyStart utiliza ventanas apiladas para presentar la pila de llamadas.
